...
public class MovimientoBean extends Persistible implements Serializable {
	...
	public String backupBajar() throws Exception {
		...
		try {
			sb.append("movimiento"); 			sb.append(";");
			...
			throw new Exception("MovimientoBean " + this.toString() + e.getMessage());
		}
	...
	public String getPeriodoSort() {
		if(cliente.getDelegacion().getBanco().isNacion())
			return DateTimeUtil.getPeriodoNacionSort(this.fecha);
		else
			return DateTimeUtil.getPeriodoSort(this.fecha);
